...
1. **이 작업만 새 모델에 맡기기**: 다른 모델에 시키려던 요청을 그대로 메시지로 전송하세요. 해당 모델의 서브에이전트가 처리하며, 메인 세션의 모델과 캐시는 그대로 유지됩니다.
...
> [!WARNING]
> 데스크톱 앱의 UI 모델 선택기에서 동일 모델을 다시 클릭하여 확인하려 하지 마세요. 전환이 차단된 후에도 UI 선택기에는 새 모델이 선택된 것처럼 표시되지만 실제 세션은 이전 모델로 유지됩니다. 이 상태에서 다시 클릭해도 요청이 전송되지 않습니다. 표시가 어긋난 경우 현재 세션의 실제 모델을 UI에서 다시 선택해 동기화해 주세요.
...
### 이 작업만 다른 모델로 위임하기
전환이 차단되었을 때 더 저렴한 대안을 제시합니다. **다른 모델에 맡기려던 프롬프트를 다음 메시지로 그냥 입력하면, 해당 모델의 서브에이전트가 단발성으로 작업을 수행합니다.**
- 세션의 메인 모델과 캐시는 그대로 유지됩니다.
- `UserPromptSubmit` Hook이 메인 모델에게 해당 메시지를 서브에이전트(Agent 도구, 예: `model: "claude-fable-5-1"`)에 위임하도록 지시합니다.
...
> [!NOTE]
> 2026-09-23 Opus 5.5 메인 세션에서 정상 위임을 확인했습니다 (`Agent`, `model: "sonnet"` 실행 후 "This session is still on Opus" 응답). 단, 메인 모델이 Haiku 4.5일 때는 위임 지시를 3회 모두 무시하는 현상이 있었습니다. 이 제안은 메인 모델의 지시 준수 능력에 의존합니다.
