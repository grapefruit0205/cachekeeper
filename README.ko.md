...
전환을 막을 때 더 싼 방법도 함께 제안합니다. **다른 모델에 맡기려던 요청을 다음 메시지로 그냥 보내면, 그 모델의 서브에이전트가 처리합니다.** 세션은 원래 모델을 유지하고, 캐시도 그대로 살아 있습니다. `UserPromptSubmit` hook이 메인 모델에게 그 메시지 하나를 서브에이전트에 맡기라고 지시합니다(Agent 도구, `model: "fable"`). 서브에이전트는 대화를 보지 못하므로 메인 모델이 필요한 내용을 정리해 넘깁니다. 그다음 메시지부터는 평소대로 처리됩니다. 안내문의 `/model` 명령을 입력하면 기존처럼 세션 모델 자체를 바꿉니다.
...
| 변수 | 기본값 | 효과 |
|---|---|---|
| `CACHEKEEPER_KEEPALIVE` | 꺼짐 | `1`이면 켜짐 |
