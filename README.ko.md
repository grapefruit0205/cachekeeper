...
> [!NOTE]
> [실측]은 제작자 컴퓨터에서 잰 값, [기록 재생]은 제작자 기록으로 계산한 값([기대 효과](#기대-효과))이고, 다른 도구와의 비교는 2026-09-24 기준 각 README([다른 Keep-Alive 도구와의 비교](#다른-keep-alive-도구와의-비교))에 따른 것입니다.

1. **설치만 하면 모든 세션에서 자동으로 동작합니다. 데스크톱 앱도 포함입니다.** 세션마다 켤 필요가 없습니다. [실측] 2026-09-23 14:50부터 09-24 17:28(KST)까지 데스크톱 앱에서 핑 26번, 가드 질문 7번, 서브에이전트 위임 1번이 기록됐습니다. 다른 도구는 세션마다 켜야 하거나(cachebeat의 `/cachebeat`, cache-tax의 `/keepwarm`. cache-tax는 세션 시작마다 켜지게 설정 가능), CLI 대화형 세션 전용이거나(demouo 등 플러그인 모니터), 얼리 액세스 기능인 function hooks가 필요한 경우(Delitefully, cache-tax)가 많습니다.
2. **휴식과 모델 전환을 둘 다 막습니다.** [기록 재생] 제작자 13일 기록에서 재구축으로 나간 사용량은 모델 전환 뒤 11.9%, 1시간 넘는 휴식 뒤 11.5%로 비슷합니다. keep-alive만으로는 이 중 절반만 다룹니다.
3. **모델 전환을 거절해도 그 모델의 답을 받을 수 있습니다.** 요청 하나만 그 모델의 서브에이전트에 맡기고, 메인 대화는 살아 있는 캐시로 이어집니다. Claude Code 자체 확인 창이 없는 데스크톱 앱과 `-p` 모드에서는 가드가 전환을 멈춰 줍니다.
4. **내 휴식 패턴으로 정책을 정하고, 구독 사용량 기준으로 셉니다.** 어떤 세션을 얼마나 유지할지 매일 내 기록으로 다시 계산합니다. 다른 도구는 고정 시간이나 직접 정한 시간을 쓰고, 핑 비용을 따지는 도구는 API 정가 기준입니다. 다만 실제 가동 시간에서는 고정 정책과의 차이가 작습니다. [기록 재생] 3시간 +6.2%, 24시간 +6.6%.
5. **절감률을 숫자로 보여 줍니다.** `cachekeeper audit`가 내 기록을 재생합니다. [기록 재생] 제작자 기록에서 keep-alive 순효과는 +6.6%(실제 가동 시간 반영)이고, 가드가 물었을 전환 뒤 재구축은 사용량의 8.2%였습니다. 재구축마다 원인도 나눠 보여 줍니다.
6. **안전장치를 CI로 검증합니다.** 모델을 깨우는 exit 2는 핑을 보낼 때만 나오므로, 설치가 깨져도 무한 반복이 생기지 않습니다([keep-alive](#keep-alive)). CI는 Linux와 macOS, 그리고 Windows의 Git Bash·PowerShell 7·Windows PowerShell 5.1에서 Hook을 실행합니다.
...
7. **근거의 범위**: 이 README의 수치는 제작자 한 명의 Linux 컴퓨터 기록입니다. Windows는 CI로만 테스트했고, Claude Code가 설치된 실제 Windows 기기에서는 아직 확인하지 못했습니다.
